...
def initial_condition(x) -> torch.Tensor:
    res = torch.sin( 2*np.pi * x).reshape(-1, 1) 
    return res
...
def compute_loss(
    ...
    interior_loss = dfdt(nn_approximator, x, t, order=1) - 3 * dfdx(nn_approximator, x, t, order=2)
...
def plot_solution(nn_trained: PINN, x: torch.Tensor, t: torch.Tensor):
    ...
    ani = FuncAnimation(fig, animate, frames=n_frames, interval=200, repeat=False)
    writergif = matplotlib.animation.PillowWriter(fps=1)
    ani.save('heat.gif',writer=writergif)
    plt.show()
...
if __name__ == "__main__":
    ...
    nn_approximator = PINN(4, 50, pinning=True)
