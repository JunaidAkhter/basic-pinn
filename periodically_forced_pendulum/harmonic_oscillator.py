...
def f(nn: NNApproximator, x: torch.Tensor) -> torch.Tensor:
    ...
    return F0 + (1 - torch.exp(-(x - boundary))) * nn(x)
...
def compute_loss(
    ...
    interior_loss = df(nn, x, order=2) +  ω* f(nn, x)
...
domain = [0.0, 10.0]
x = torch.linspace(domain[0], domain[1], steps=200, requires_grad=True)
x = x.reshape(x.shape[0], 1)

nn_approximator = NNApproximator(6, 60)
